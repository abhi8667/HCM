...
def train_nn_bce(model, X_tab_tr, X_esm_tr, y_tr, epochs=40, lr=0.003):
...
def main():
    print("=== Starting BCE + Platt Scaling Model Training ===")
    ...
    # We use the best configuration from the grid search
    hidden_size = 32
    esm_dim = 1280
    ...
    for target_gene in target_genes:
        ...
        X_esm_tr, X_esm_te = X_esm_full[train_idx], X_esm_full[test_idx]
        ...
        model = train_nn_bce(model, X_tab_tr, X_esm_tr, y_tr, epochs=40, lr=0.003)
    ...
    df_res.to_csv('bce_model_results.csv', index=False)
    
    # Calculate average
    avg_df = df_res.drop(columns=['Gene', 'Model']).mean()
    print("\\nAverage Metrics:")
    print(avg_df)
